...
acc_gbm = accuracy_score(y_test, model_gbm.predict(X_test))

# Wagi proporcjonalne do accuracy
total_acc = acc_rf + acc_tree + acc_knn + acc_gbm
w_rf = acc_rf / total_acc
w_tree = acc_tree / total_acc
w_knn = acc_knn / total_acc
w_gbm = acc_gbm / total_acc

print(f"Wagi modeli (RF: {w_rf:.2f}, Tree: {w_tree:.2f}, kNN: {w_knn:.2f}, GBM: {w_gbm:.2f})")
...
evaluate_model(y_test, model_gbm.predict(X_test), "Gradient Boosting")
